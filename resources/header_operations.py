...
#功能：设置对象的值，不能用于sliders, combo buttons和check boxes
...
###显示物品细节修饰语 show_default_text_or_not为1显示“默认”的默认项目费用
...
###显示日志消息，同display_message不同的是显示消息的同时保存到log里可以查看
...
###troop减少钱 建议使用带声音的函数(call_script, "script_troop_add_gold", "trp_player", 5000),
...
###获取堆troop的dna复制品（使用dna可以省去移除首领加入首领等操作？）
...
###获取堆囚犯dna复制品（使用dna可以省去移除首领加入首领等操作？）
...
###设置队伍icon，相关icon参考module_map_icons.py
...
###mordr_hold坚守 mordr_stand_closer聚拢 mordr_stand_ground站岗 mordr_charge进攻 mordr_spread_out散开
...
###(add_missile, <agent_id>, <起始坐标>, <起始速度固定点>, <武器物品id>, <武器物品修饰语>, <物品触发器id>, <物品触发器修饰语>),
...
###屏幕切换到交易界面，当在对话中module_dialogs.py使用，需要多增加方括号。例如 [change_screen_trade]
...
###屏幕切换到交易俘虏的界面，当在对话中module_dialogs.py使用，需要多增加方括号。例如 [change_screen_trade_prisoners]
change_screen_trade_prisoners          = 2044	# (change_screen_trade_prisoners),
###屏幕切换到买雇佣兵的界面，当在对话中module_dialogs.py使用，需要多增加方括号。例如 [change_screen_buy_mercenaries]
change_screen_buy_mercenaries          = 2045	# (change_screen_buy_mercenaries),
###屏幕切换到查看角色情况的界面，当在对话中module_dialogs.py使用，需要多增加方括号。例如 [change_screen_view_character]
...
###设定触发器的返回值 (set_trigger_result,1)可以按Tab键离开，默认为0无法按Tab键离开
...
###设置agetn速度修饰语（值是百分比，默认值100，范围0-1000）
...
###设置agetn再装填速度修饰语（值是百分比，默认值100，范围0-1000）
...
###设置agent使用速度修饰语（值是百分比，默认值100，范围0-1000）
...
###获取一个随机数（废弃，使用store_random_in_range）
...
###获取troop有多少钱 
###例：获取玩家的钱数 (store_troop_gold, ":gold", "trp_player"),
...
###获取troop阵营 (store_troop_faction, ":cur_faction", ":cur_troop"),
