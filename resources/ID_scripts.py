...
###谈话信息内容设置（即谈话时鼠标移动到npc框出现一个关系好感面板里的内容）
...
###获取组兵种比例（输出 reg0步兵 reg1射手 reg1骑兵）
script_team_get_class_percentages = 311
###获取敌军坐标最靠近距离（输出 reg0平均距离 reg1最小距离）
...
###关系描述到s63（即谈话时鼠标移动到npc框出现一个关系好感面板里的s63内容，script_setup_talk_info的后续）
...
###获取玩家队伍最大技能（reg0为最大技能值，reg1为最大技能是那个人）
script_get_max_skill_of_player_party = 422
###提升领主队伍经验
script_upgrade_hero_party = 423
###获取建造详细资料（输出s0为建造名称、输出reg0为建造成本，s1为建筑的详细功能说明）
...
###检测自立后开宴会箱子里的食物、酒、香料等有多少。输出到s9。   (call_script, "script_internal_politics_rate_feast_to_s9", "trp_household_possessions", 600, "$players_kingdom", 0),
...
##战斗中中立者的行为（越狱和潜入失败逃跑时用于城镇中心战斗触发器）
...
###更新地图命令面板
script_update_order_panel_map = 569
